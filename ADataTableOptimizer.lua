        ...
        tableRef = {
            ...
            postOutput = function( ostream )
                if defaultValues then
                    ...
                    outFile.write(string.format("function %sGet(id)\n",datasetName));
                    outFile.write(string.format("    if(not %s[id]) then\n",datasetName));
                    outFile.write(string.format('       LogError("配置%s没有id:",id)\n',datasetName));
                    outFile.write("    end\n");
                    outFile.write(string.format("    return %s[id]\n",datasetName));
                    outFile.write("end\n");
                    

                    --outFile:write(testStr)
--                    outFile:write( string.format("       LogError(\"配置%s没有id:\",id \n end \n)",datasetName))
--                    outFile:write( string.format("       return %s[id]\",id \n end \n)",datasetName))
                end
            end
        }
    end
...
local function ExportDatabaseLocalText( tofile, newStringBank )
    ...
    for _, v in ipairs( files ) do
        if GenerateFiles[ v ] then
            local ret=LoadDataset( v )
            if ret==false then
                print("已经生成过了")
            else
                local t = Database[ v ]
